...
function SourceMark({ source, className = "" }) {
    const ActiveSource =
        SOURCE_TABS.find((tab) => tab.id === source) || SOURCE_TABS[2];

    return (
        <span className={`link-summary-source-mark ${source} ${className}`}>
            <ActiveSource.icon size={16} aria-hidden="true" />
        </span>
    );
}
...
export default function LinkSummaryPage() {
    ...
    const activeTab =
        SOURCE_TABS.find((tab) => tab.id === sourceType) || SOURCE_TABS[0];
    ...
    const sourceLabel =
        SOURCE_TABS.find((tab) => tab.id === detectedSource)?.label ||
        "Web URL";

    const sourceBadgeLabel =
        sourceLabel === "Web URL" ? "Web URL" : `${sourceLabel} URL`;

    const displayTitle =
        asText(
            getNestedValue(summary, [
                "title",
                "summary_title",
                "topic",
                "page_title",
    ...
    const generatedDate =
        asText(
            getNestedValue(summary, ["date", "generatedAt", "generated_at"]),
            ""
        ) || (summary ? getTodayLabel() : FALLBACK_SUMMARY.generatedAt);

    const generatedTime =
        asText(getNestedValue(summary, ["time", "generated_time"]), "") ||
        (summary ? getTimeLabel() : "12:45 PM");
    ...
    const keyPoints = asList(
        ...
        normalizedSummary.keyPoints?.length
            ? normalizedSummary.keyPoints
            : FALLBACK_SUMMARY.keyPoints
    );
    ...
    const runGenerateSummary = async () => {
        ...
        try {
            const { data } = await axiosClient.post("/ai/link-summary", {
                url: trimmedUrl,
            });
    ...
    const handleSaveSummary = async () => {
        ...
        try {
            await axiosClient.post("/summaries", {
                title: summaryTitle,
                source_type: "text",
                summary_text: serializeStudySheetSummary(
                    summary,
                    summaryContext
                ),
            });
    ...
    const handleCopySummary = async () => {
        ...
        try {
            await navigator.clipboard.writeText(
                formatStudySheetAsText(summary, summaryContext)
            );
        ...
        <div className="link-summary-page">
            <header className="link-summary-header">
                <div>
                    ...
                    <p>
                        Paste an Instagram, YouTube, or webpage URL to generate
                        a clean AI summary.
                    </p>
                </div>
            ...
            <div className="link-summary-layout">
                <main className="link-summary-main">
                    <form
                        className="link-summary-input-card"
                        onSubmit={handleGenerateSummary}
                    >
                        <div
                            className="link-summary-tabs"
                            role="tablist"
                        ...
                        >
                            {SOURCE_TABS.map((tab) => (
                                <button
                                    ...
                                    className={`link-summary-tab ${
                                        sourceType === tab.id ? "active" : ""
                                    }`}
                                    key={tab.id}
                        ...
                        <div className="link-summary-url-row">
                            ...
                            <button
                                className="link-summary-generate-button"
                                disabled={loading}
                                type="submit"
                            >
                    ...
                    <section
                        className="link-summary-stats"
                        aria-label="Summary statistics"
                    >
                        {SUMMARY_STATS.map((stat) => (
                            <article
                                className="link-summary-stat-card"
                                key={stat.label}
                            >
                                <span
                                    className={`link-summary-stat-icon ${stat.tone}`}
                                >
                    ...
                    <section
                        className="link-summary-result-card"
                    ...
                    >
                        <div className="link-summary-result-top">
                            ...
                            <div className="link-summary-result-heading">
                                ...
                                <div className="link-summary-badges">
                                    <span className="link-summary-source-badge">
                                        Source: {sourceBadgeLabel}
                                    </span>
                                    {summaryType && (
                                        <span className="link-summary-soft-badge">
                                            {summaryType}
                                        </span>
                                    )}
                                </div>
                        ...
                        <div className="link-summary-result-footer">
                            ...
                            {(saveMessage || copyMessage) && (
                                <p className="link-summary-action-message">
                                    {saveMessage || copyMessage}
                                </p>
                            )}
                        </div>
                ...
                <aside
                    className="link-summary-side"
                    aria-label="Summary sidebar"
                >
                    <section className="link-summary-side-card">
                        ...
                        <div className="link-summary-recent-list">
                            {RECENT_SUMMARIES.map((item) => (
                                <article
                                    className="link-summary-recent-item"
                                    key={item.title}
                                >
                                    <div
                                        className={`link-summary-mini-thumb ${item.tone}`}
                                    >
                                        <item.icon
                                            size={18}
                                            aria-hidden="true"
                    ...
                    <section className="link-summary-side-card">
                        ...
                        <div className="link-summary-saved-list">
                            {SAVED_LINKS.map((item) => (
                                <article
                                    className="link-summary-saved-item"
                                    key={item.title}
                                >
                                    <div
                                        className={`link-summary-saved-icon ${item.tone}`}
                                    >
                                        <item.icon
                                            size={17}
                                            aria-hidden="true"
                ...
                <div className="saved-links-panel">
                    <div className="saved-links-panel-card">
                        <div className="saved-links-panel-header">
                            ...
                            <button
                                type="button"
                                onClick={() => setShowSavedLinksPanel(false)}
                            >
                                ×
                            </button>
                        </div>
