...
def _question_too_generic(question: str) -> bool:
    ...
    if any(pattern in q for pattern in weak_patterns) and len(q.split()) < 9:
        return True
...
def _repair_or_reject_answer_mismatch(question: dict, best_evidence: str) -> tuple[bool, str, dict]:
    ...
    # If evidence clearly supports another answer, repair the letter.
    if best_letter != claimed_letter:
        if best_score >= claimed_score + 0.07:
            fixed_question["correct_answer"] = best_letter
            fixed_question["validation_repair"] = {
                "repaired": True,
                "old_correct_answer": claimed_letter,
                "new_correct_answer": best_letter,
                "reason": "nomic-embed-text found that source/explanation supports a different option.",
                "scores": scores,
                "model": EMBEDDING_MODEL,
            }
            return True, "Correct answer letter repaired by embedding", fixed_question

        return False, f"Correct answer mismatch but not safe to repair. Scores: {scores}", question
...
def _validate_mcq_nlp(question: dict, context: str) -> tuple[bool, str, dict]:
    ...
    best_evidence = source if len(source) >= 30 else _best_evidence_for_question(
        q_text,
        evidence_query_text,
        context,
    )
    ...
    support_score = _nlp_overlap_score(
        correct_text + " " + explanation,
        best_evidence,
    )

    # Keep this threshold gentle because the embedding verifier already does the heavy check.
    if support_score < 0.03:
    ...
    fixed_question["validation"] = {
        ...
        "method": "nomic_embedding_validator_with_answer_repair",
    }
...
def _validate_mcq_with_llm_judge(question: dict, context: str) -> tuple[bool, str, dict]:
    ...
    if clean_supported[0] != correct_letter:
        fixed_question = dict(question)
        fixed_question["correct_answer"] = clean_supported[0]
        fixed_question["validation_llm_judge"] = {
            "repaired": True,
            "old_correct_answer": correct_letter,
            "new_correct_answer": clean_supported[0],
            "reason": data.get("reason", "LLM judge found a different supported answer."),
        }
        return True, "LLM judge repaired correct answer", fixed_question
...
- If the question asks about a difference/comparison, the correct option must mention both sides of the comparison.
